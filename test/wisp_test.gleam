...
pub fn multipart_form_files_too_big_test() {
  let limit_test = fn(limit, callback) {
    "--theboundary\r
Content-Disposition: form-data; name=\"two\"; filename=\"file.txt\"\r
\r
12\r
...
"
  ...
  limit_test(1, fn(_) { panic as "should be unreachable for limit of 1" })
  |> should.equal(Response(413, [], wisp.Empty))

  limit_test(2, fn(_) { panic as "should be unreachable for limit of 2" })
  |> should.equal(Response(413, [], wisp.Empty))

  limit_test(3, fn(_) { panic as "should be unreachable for limit of 3" })
  |> should.equal(Response(413, [], wisp.Empty))

  limit_test(4, fn(_) { Nil })
  |> should.equal(Response(200, [], wisp.Empty))
}
